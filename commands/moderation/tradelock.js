...
module.exports.run = (client, message, args, level, Discord) => {
  ...
  categories.forEach((cat) => {
    ...
    if (catObj.permissionsFor(client.config.tradeRole).has('SEND_MESSAGES')) {
      ...
      catObj.updateOverwrite(client.config.tradeRole, { SEND_MESSAGES: false }, 'Lock Channel for Maintenance')
        .then((c) => {
      ...
      catObj.updateOverwrite(client.config.tradeRole, { SEND_MESSAGES: true }, 'Unlock Channel After Maintenance')
        .then((c) => {
          c.children.forEach((channel) => {
            channel.send(msgToSend);
          });
