module.exports.run = (client, message, args, level) => {
  ...
  const index = client.raidJoins.indexOf(message.guild.members.get(args[0]));
  if (index > -1) {
    const removed = client.raidJoins.splice(index, 1);
    client.raidMembersPrinted -= 1;
    client.success(message.guild.channels.get(client.getSettings(message.guild).staffChat),
